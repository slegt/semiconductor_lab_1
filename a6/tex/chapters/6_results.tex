...
The $(036)$ reflex is related to both $a$ and $c$ lattice constants of the substrate
as
\[
	\lvert \mathbf{q}\rvert =\sqrt{ 3 \cdot \frac{4 \pi}{\sqrt{ 3 }a} + 6 \cdot \frac{2\pi}{c}
	}
\]
Substituting $c$ from above yields an $a=4.75925\pm0.00001 nm$. No thin film $a$
constant can be found in the spectrum.
...
A frame-based reciprocal space map was recorded around the symmetrical $(024)$
sapphire reflection, see \cref{fig:task4_rsm}. The left hand side shows the full
map, while the right hand side displays the the region of interest. A high
intensity peak can be ovserved at $q_{\para}= \qty{3.61}{\angstrom}$ , a broader
peak below at $q_{\para}= \qty{3.549}{\angstrom}$, corresponding to the thin film.
Using $c_{\mathrm{film}}=1.3042\pm0.0005 nm$ from the task above, the $a$ lattice
constant can be calculated as:
\begin{equation}
	q^{2}=k^{2}\frac{16 \pi^{2}}{3a^{2}}+l^{2}\frac{4\pi^{2}}{c^{2}}
\end{equation}
The broadening in $q_{\parallel}$ direction originates from the finite thickness
of the respective layer. Broadening in $q_{\perp}$ of a symmetrical reflex originates
from the lateral coherence length of the individual crystallites as well as the film
mosaicity.
