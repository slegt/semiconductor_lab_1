...
Understanding it requires first considering the simple hexagonal structure, defined
by the primitive vectors \autocite{Ashcroft}:
\begin{equation}
...
empty spaces exist between the closely packed oxygen atoms. Since every empty space
...
above one another, leading to a hexagonal arrangement of interstices.
...
The holes in adjacent meshes are shifted relative to each other along the close-packed
direction of the oxygen hcp lattice by a magnitude of an oxygen-oxygen distance. This
out-of-plane shift of the holes is depicted in \cref{fig:corundum_3}. Since a
translation of three out-of-plane shift vectors results in an identical
arrangement, three different hole layers exist, labeled A, B, and C. The oxygen hcp
meshes are labeled with '1' and '2'. The structural repeat unit of the corundum
structure is then given by the sequence 1A2B1C2A1B2C consisting of six oxygen
layers.
...
found such that the plane equation $\mathbf{r}\cdot \mathbf{x}= A$ is satisfied. By
...
_{3}$, the plane equation $\mathbf{r}\cdot (x_{i}\mathbf{a}_{i})=A$ is satisfied,
and with $\mathbf{r}\cdot \mathbf{a}_{1}=2\pi h$,
$\mathbf{r}\cdot \mathbf{a}_{2}=2\pi k$, $\mathbf{r}\cdot \mathbf{a}_{3}=2\pi l$,
...
\begin{equation}
	...
	a_{\mathrm{substrate}}}
\end{equation}
In this equation, $a_{\mathrm{film}}$ is the measured lattice constant of a thin film
and $a_{\mathrm{film}}^{\mathrm{relax}}$ is the fully relaxed lattice constant.
$r=0$ corresponds to pseudomorphic growth, while $r=1$ corresponds to a fully relaxed
film.
