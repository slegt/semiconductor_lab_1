...
lattice. The distances between adjacent lattice points, i.e., the magnitudes $\lvert
...
\begin{figure*}
	...
	\begin{subfigure}
		...
		\caption{Hexagonal close-packed (hcp) lattice.}
		\label{hcp}
	\end{subfigure}
	\par
	\begin{subfigure}
	...
	\end{subfigure}
	\caption{Selected crystal lattices for investigating the corundum structure. (c)--(e)
	adapted from \autocite{kronberg_plastic_1957}.}
	\label{fig:lattice_structures}
\end{figure*}
...
spaces are referred to as octahedral interstices and are depicted in \cref{fig:corundum}.
...
in-plane solution maximizing the separation of the holes due to their negative
charge. To fulfill this condition in-plane, the holes are arranged in a wider
hexagonal ``hole'' mesh as a subset of the interstitial mesh, shown in \cref{fig:corundum_2}.
...
the close-packed direction of the oxygen hcp lattice by one oxygen-oxygen distance.
This out-of-plane shift of the holes is depicted in \cref{fig:corundum_3}. Since a
translation of three out-of-plane shift vectors results in an identical arrangement,
three different hole layers exist, labeled A, B, and C. The oxygen hcp meshes
are labeled with `1' and `2'. The structural repeat unit of the corundum structure
is then given by the sequence 1A2B1C2A1B2C, consisting of six oxygen layers.

The corundum structure is illustrated using \ce{Al2O3} as a representative material,
with reference to the hcp oxygen sublattice and an aluminum interstitial mesh, though
these may in general be substituted by other anions/cations. \ac{algao} also crystallizes
in the corundum structure, where Al and Ga occupy the interstitial lattice
\autocite{peelaers_structural_2018}.

\subsection{Reciprocal Space}
\label{sec:reciprocal_space}

The reciprocal lattice is an important concept in the subsequent considerations
of periodic structures. The goal is to construct a function that is lattice-periodic
in the direct lattice. This function should therefore satisfy
$f(\mathbf{x})=f(\mathbf{x}+\mathbf{A})$ if
$\mathbf{A}=\sum_{i=1}^{3}\alpha_{i}\mathbf{a}_{i}$. Using a series expansion,
the following form is obtained:
\begin{align}
	f(\mathbf{x})            & =\sum_{\mathbf{B}}a_{\mathbf{B}}\cdot \exp(\mathrm{i}\mathbf{B}\cdot\mathbf{x}), \notag                                                                                                           \\
	f(\mathbf{x}+\mathbf{A}) & =\sum_{\mathbf{B}}a_{\mathbf{B}}\cdot \exp(\mathrm{i}\mathbf{B}\cdot \mathbf{x})\cdot \underbrace{ \exp(\mathrm{i}\mathbf{B}\cdot \mathbf{A}) }_{ \stackrel{!}{=}1 }\stackrel{!}{=}f(\mathbf{x}).
\end{align}
This implies the necessary condition $\exp(\mathrm{i}\mathbf{B}\cdot \mathbf{A})=
1$, which is equivalent to $\mathbf{B}\cdot \mathbf{A}=2\pi z$ with
$z \in \mathbb{Z}$. It is possible to define the reciprocal lattice as the set $\mathcal{G}
=\{\mathbf{B}\,\vert\, \exp(\mathrm{i}\mathbf{B}\cdot \mathbf{A})=1 \quad \forall
\mathbf{A}\in \text{span}(\mathbf{a}_{i}) \}$ \autocite{Ashcroft}. Analogously to
real space, primitive vectors can also be formed as:
\begin{equation}
...
\label{sec:lattice_planes_and_directions} This section is based on the findings of
\citeauthoryear{Ashcroft}.
...
these vectors, $\mathbf{b}$, is chosen, which always has a length of $2 \pi / d$.
The converse is also true: for every vector $\mathbf{B}$ of the reciprocal lattice,
...
$\mathbf{b}= h \mathbf{b_1}+ k \mathbf{b_2}+ l \mathbf{b_3}$. The entries of the tuple
$(h\,k\,l )$ are the Miller indices, which by definition must be integers.
...
\begin{figure*}
	...
	\end{subfigure}
	\caption{Heteroepitaxial thin film growth. Adapted from \autocite{wsi2007hrxrd}.}
	\label{fig:film_growth}
\end{figure*}

This subsection is based on \autocite{wsi2007hrxrd}.

As the crystal exchanges energy with its environment, it minimizes the free energy
$F=U-TS$ instead of its internal energy $U$. This introduces deviations from the
ideal crystal structure, i.e. defects. A small concentration of defects can fundamentally
influence the crystal's properties.

Most thin films differ chemically from their substrate. In that case, the film grows
heteroepitaxially, as depicted in \cref{fig:film_growth}. If the lattice constants
do not differ too much, the film adopts the substrate's in-plane lattice
constant $a$. This is called pseudomorphic growth. A figure of merit to quantify
the degree of relaxation is given as:
\begin{equation}
	r = \frac{a_{\mathrm{film}}- a_{\mathrm{substrate}}}{a_{\mathrm{film}}^{\mathrm{relax}}-
	a_{\mathrm{substrate}}}.
\end{equation}
In this equation, $a_{\mathrm{film / substrate}}$ is the measured in-plane
lattice constant of a thin film / substrate, and $a_{\mathrm{film}}^{\mathrm{relax}}$
is the fully relaxed lattice constant. $r=0$ corresponds to pseudomorphic growth,
while $r=1$ corresponds to a fully relaxed film. $r<1$ induces material strain.

As the strain of the film increases with each deposited layer, strain energy is released
by one-dimensional defects: dislocations. The dislocation's orientation is characterized
by a dislocation vector $\mathbf{L}$. In most cases, dislocations appear parallel
to the growth direction. When moving in a loop around the dislocation using the
local lattice coordinate system, the starting point is not reached. The
difference between the start and end points is defined by the Burgers vector
$\mathcal{B}$, depicted in \cref{fig:dislocations}. Depending on the orientation
of $\mathcal{B}$ relative to $\mathbf{L}$, one can distinguish between screw-type
dislocations ($\mathcal{B}\parallel \mathbf{L}$) and edge-type dislocations ($\mathcal{B}
\perp \mathbf{L}$).

Edge-type dislocations prefer to group together in order to minimize the total strain
energy. This agglomeration of dislocations results in grain boundaries that
separate monocrystalline regions of the thin film, called crystallites. They are
twisted by an angle $\delta$. By analogous reasoning, an agglomeration of screw-type
dislocations results in a tilt of the crystallites.

\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{../assets/dislocation_types.pdf}
	\caption{Dislocations of edge (left) and screw (right) type. Image taken from \autocite{fleck_burgers_vector}.}
