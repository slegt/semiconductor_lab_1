...
for x in range(4):
    for y in range(2):
        tikz.rectangle(
            left_corner=(x * strained_width, 3 * substrate_width + y * film_width),
            width=strained_width,
            height=film_width,
...
thin_film_width = tikz.line(
    ...
    end=(strained_width, 3 * substrate_width + 2 * film_width + padding),
    options="|-|",
)
tikz.node(position=thin_film_width.midpoint(), text="$a_\mathrm{film}^\mathrm{relax}$", options="above")

tikz.compile(pdf_destination= file_path.parent / "assets" / "fully_relaxed.pdf")
